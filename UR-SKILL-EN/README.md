...
**UR-SKILL turns fuzzy natural language requirements into structured, verifiable AI Agent skill packages or system prompts -- the project's own 3 internal agents (research-analyst, tech-documentation, script-engineer) and its translation skill `cn-en-tech-translator` are all built using the UR-SKILL methodology (eating our own dog food).**
...
| **Self-Iterating** | Eating our own dog food -- 3 internal agents + translation skill `cn-en-tech-translator` are all built with the UR-SKILL methodology, verified by 135 tests + 87% coverage CI before release |
...
> **Automatic Platform Tool Adaptation**: UR-SKILL identifies your current platform in Step 2 and verifies tool references in Step 10. When a target platform is specified (e.g., "generate for Cursor"), it references the platform mapping table in [skill-package-design-guide.md §A](design-guides/skill-package-design-guide.md) for tool specialization; when unspecified, the LLM directly perceives available tools on the current platform and adapts automatically. See [skill-package-design-guide.md §A](design-guides/skill-package-design-guide.md).
